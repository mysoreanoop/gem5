        ...
        for k, v in kwargs.items():
            ...
            def set_single(x, v):
                if isinstance(v, str):
                    ...
                    set_nested_dict(x, v)
                    x.nested_type = (
                        pb2.DebugAnnotation.NestedValue.NestedType.DICT
                    )
                    ...
                    set_nested_list(x, v)
                    x.nested_type = (
                        pb2.DebugAnnotation.NestedValue.NestedType.ARRAY
                    )
                else:
                    assert False
    ...
    def _add_debug_annotation_new(self, d, kwargs):
        ...
        for k, v in kwargs.items():
            ...
            if cnt == self.list_max_size:
                x.name = "..."
                x.string_value = f"({len(kwargs) - cnt} more items)"
                break
