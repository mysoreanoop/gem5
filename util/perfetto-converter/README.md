...
This is based on https://github.com/ihavnoid/tg4perfetto.
Their README is below.
This section describes usage for AMD gem5 users.

## Setup
Install the requirements and this project using pip3.
IMPORTANT: If you are on ETX/LSF/ATL you MUST use pip3.
If your system has python-is-python3 I believe it doesn't matter.
...
## Usage [WIP]
Run gem5 with Perfetto logging on.
To do this the configs must be changed such that Shader.use_perfetto = True.

Navigate to the `src/tg4perfetto/` directory (util/perfetto-converter/src/tg4perfetto).
Run `./convert.py` using the option `-i` to specify the path to the m5out folder with a perfetto-log.gz.
You may also specify an output trace using the `-o` option and further restrict the trace to specific AQL/SDMA tasks.
Use `./convert.py -h` for a full set of options.

With the output trace created move it to a computer with a GUI web browser.
Open a web browser and go to [https://ui.perfetto.dev/](https://ui.perfetto.dev/).
Click "Open Trace File" and open the output trace file.

## Features
With a trace open in a web browser you can use the Perfetto hotkeys to zoom/select/click on slices.
Clicking on a slice displays some information in the bottom right.
Tasks show start/end time, grid/wg size, kernel name.
SDMA copies show src/dest address, size, and calculated transfer rate.
Instructions show exec mask, disassembly, and reason for instruction stall (if any).
